...
setwd("~/Desktop/Michigan/503_multivariate/project/data_cleaning/variables_dummy")
...
uk_acc$hit_object_off_carriageway<-NULL
...
####################################
# CHANGE TO FACTORS TO DUMMY
#####################################

sapply(uk_acc, class)


numeric<-uk_acc[,c("accident_index","age_of_casualty", "longitude", "latitude",
                   "age_of_driver","age_of_vehicle")]

factors<-uk_acc[,-match(c("accident_index","age_of_casualty", "longitude", "latitude",
                          "age_of_driver","age_of_vehicle"), names(uk_acc))]


mm<-with(factors, data.frame(model.matrix(~sex_of_casualty + day_of_week)))
